 ...
 * zero occupancy value, i.e. no cachelines are tagged with this RMID,
 * once __intel_cqm_rmid_rotate() returns.
 */
 ...
 * list.
 *
 * These lists are protected by cache_mutex.
 */
...
static bool __match_event(struct perf_event *a, struct perf_event *b)
{
	...
	/*
	 * Events that target tasks in the same thread group are placed
	 * into the same cache group.
	 */
	if (task_tgid_nr(a->hw.cqm_target) == task_tgid_nr(b->hw.cqm_target))
		return true;
 ...
 * evicted.
 *
 * On successful rotations we decrease the threshold back to zero.
 */
